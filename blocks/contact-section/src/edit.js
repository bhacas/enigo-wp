				...
				<PanelBody title={__('Contact Items', 'contact-section')}>
					{contactItems.map((item, index) => (
						<div key={index} style={{ marginBottom: '1em', paddingBottom: '1em', borderBottom: '1px solid #ccc' }}>
							...
							<TextareaControl label={__('SVG Icon Code', 'contact-section')} value={item.icon} onChange={(val) => handleItemChange(index, 'icon', val)} rows="2" />
							<TextControl label={__('Headline', 'contact-section')} value={item.headline} onChange={(val) => handleItemChange(index, 'headline', val)} />
							<TextControl label={__('Value', 'contact-section')} value={item.value} onChange={(val) => handleItemChange(index, 'value', val)} />
			...
			<section {...blockProps}>
				<div className="container mx-auto px-4">
					<div className="text-center mb-16">
						<RichText tagName="h2" className="text-3xl md:text-4xl font-bold mt-2 mb-4 text-gray-900" value={title} onChange={(val) => setAttributes({ title: val })} placeholder={__('Get in touch', 'contact-section')} />
						<RichText tagName="p" className="text-lg text-gray-600 max-w-2xl mx-auto" value={description} onChange={(val) => setAttributes({ description: val })} placeholder={__('Got a question or a project in mind?', 'contact-section')} />
					</div>
					<div className="grid md:grid-cols-2 gap-12">
						<div>
							{contactItems.map((item, index) => (
								<div key={index} className="flex items-start mb-8">
									<div className="bg-blue-100 p-3 rounded-full mr-4" dangerouslySetInnerHTML={{ __html: item.icon }} />
									<div>
